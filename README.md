...
| [CR-BP-39](change-requests/CR-BP-39-changelog-readme-reconciliation.md) | CHANGELOG + README + docs Reconciliation | **Proposed (this PR)** | First post-tranche slice. Reconciles tracking artifacts (`CHANGELOG.md`, root `README.md`, `change-requests/README.md`) with `main` at `e371cff6` after the BP-32/33/34 tranche plan closure. Adds 7 missing `[Unreleased]` CHANGELOG entries; corrects stale "Proposed (this PR)" status rows in both READMEs. No entity, schema, validator, or gate change. |
